...
// The authenticate function is called whenever YAP requires to authenticate, for a Context for which this script
...
function authenticate(helper, paramsValues, credentials) {
	...
	// The path Wordpress sets on the session cookies is illegal according to the standard. The web browsers ignore this and use the cookies anyway, but the Apache Commons HttpClient used in YAP really cares about this (probably the only one who does it) and simply ignores the "invalid" cookies [0] , [1],so we must make sure we MANUALLY add the response cookies
