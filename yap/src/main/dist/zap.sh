...
# Base directory where YAP is installed
BASEDIR="`pwd -P`"

# Switch to the directory where YAP is installed
cd "$BASEDIR"
...
if [ "`echo ${JAVA_OUTPUT} | grep "continuing with system-provided Java"`" ] ; then
  ...
  echo "    " $JAVA_OUTPUT
  echo "Unsetting JAVA_HOME and continuing with YAP start-up"
  unset JAVA_HOME
fi
...
if [ $JAVA_MAJOR_VERSION -ge 11 ]; then
  ...
  echo "Exiting: YAP requires a minimum of Java 11 to run, found $JAVA_VERSION"
  exit 1
fi

if [ "$OS" = "Darwin" ]; then
  JVMPROPS="$HOME/Library/Application Support/YAP/.YAP_JVM.properties"
else
  JVMPROPS="$HOME/.YAP/.YAP_JVM.properties"
fi
...
# Start YAP; it's likely that -Xdock:icon would be ignored on other platforms, but this is known to work
if [ "$OS" = "Darwin" ]; then
  # It's likely that -Xdock:icon would be ignored on other platforms, but this is known to work
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -Xdock:icon="../Resources/YAP.icns" -jar "${BASEDIR}/@yapJar@" "${ARGS[@]}"
else
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -jar "${BASEDIR}/@yapJar@" "${ARGS[@]}"
fi
