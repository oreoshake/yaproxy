...
public class SpiderTask implements Runnable {
    ...
    private org.yaproxy.yap.spider.parser.SpiderResourceFound resourceFound;
    ...
    private void runImpl() {
        ...
        org.yaproxy.yap.spider.filters.ParseFilter.FilterResult filterResult =
                org.yaproxy.yap.spider.filters.ParseFilter.FilterResult.NOT_FILTERED;
        boolean wanted = false;
        for (org.yaproxy.yap.spider.filters.ParseFilter filter :
                parent.getController().getParseFilters()) {
            filterResult = filter.filtered(msg);
            if (filterResult.isFiltered()) {
                break;
            } else if (filterResult
                    == org.yaproxy.yap.spider.filters.ParseFilter.FilterResult.WANTED)
                wanted = true;
        }
    ...
    private void setErrorResponse(HttpMessage msg, Exception cause) {
        ...
        try {
            responseHeader = new HttpResponseHeader("HTTP/1.1 400 YAP IO Error");
    ...
    private void processResource(HttpMessage message) {
        List<org.yaproxy.yap.spider.parser.SpiderParser> parsers =
