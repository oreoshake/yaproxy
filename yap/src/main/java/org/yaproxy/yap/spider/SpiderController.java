 ...
 */
@Deprecated
public class SpiderController implements org.yaproxy.yap.spider.parser.SpiderParserListener {

    /** The fetch filters used by the spider to filter the resources which are fetched. */
    private LinkedList<org.yaproxy.yap.spider.filters.FetchFilter> fetchFilters;

    /**
     * The parse filters used by the spider to filter the resources which were fetched, but should
     * not be parsed.
     */
    private LinkedList<org.yaproxy.yap.spider.filters.ParseFilter> parseFilters;

    private org.yaproxy.yap.spider.filters.ParseFilter defaultParseFilter;

    /** The parsers used by the spider. */
    private LinkedList<org.yaproxy.yap.spider.parser.SpiderParser> parsers;

    private List<org.yaproxy.yap.spider.parser.SpiderParser> parsersUnmodifiableView;
    ...
    protected SpiderController(
            Spider spider, List<org.yaproxy.yap.spider.parser.SpiderParser> customParsers) {
        super();
        this.spider = spider;
        ...
        for (org.yaproxy.yap.spider.parser.SpiderParser parser : customParsers) {
    ...
    private void prepareDefaultParsers() {
        ...
        org.yaproxy.yap.spider.parser.SpiderParser parser;

        // If parsing of robots.txt is enabled
        if (spider.getSpiderParam().isParseRobotsTxt()) {
            parser =
                    new org.yaproxy.yap.spider.parser.SpiderRobotstxtParser(
        ...
        if (spider.getSpiderParam().isParseSitemapXml()) {
            ...
            parser =
                    new org.yaproxy.yap.spider.parser.SpiderSitemapXMLParser(
                            spider.getSpiderParam());
            parsers.add(parser);
        } else {
        ...
        if (spider.getSpiderParam().isParseSVNEntries()) {
            parser =
                    new org.yaproxy.yap.spider.parser.SpiderSVNEntriesParser(
                            spider.getSpiderParam());
            parsers.add(parser);
        }
        ...
        if (spider.getSpiderParam().isParseGit()) {
            parser = new org.yaproxy.yap.spider.parser.SpiderGitParser(spider.getSpiderParam());
            parsers.add(parser);
        }

        // Redirect requests parser
        parser = new org.yaproxy.yap.spider.parser.SpiderRedirectParser(spider.getSpiderParam());
        parsers.add(parser);

        // HTTP Header parser
        parser = new org.yaproxy.yap.spider.parser.SpiderHttpHeaderParser(spider.getSpiderParam());
        parsers.add(parser);

        // Simple HTML parser
        parser = new org.yaproxy.yap.spider.parser.SpiderHtmlParser(spider.getSpiderParam());
        this.parsers.add(parser);

        // HTML Form parser
        parser =
                new org.yaproxy.yap.spider.parser.SpiderHtmlFormParser(
        ...
        parser = new org.yaproxy.yap.spider.parser.SpiderODataAtomParser(spider.getSpiderParam());
        this.parsers.add(parser);

        // Prepare the parsers for simple non-HTML files
        parser = new org.yaproxy.yap.spider.parser.SpiderTextParser(spider.getSpiderParam());
        this.parsers.add(parser);
    ...
    protected void addSeed(URI uri, String method) {
        org.yaproxy.yap.spider.parser.SpiderResourceFound resourceFound =
        ...
        spider.notifyListenersFoundURI(
                uri.toString(),
                method,
                org.yaproxy.yap.spider.filters.FetchFilter.FetchStatus.SEED);
    }
     ...
     */
    protected LinkedList<org.yaproxy.yap.spider.filters.FetchFilter> getFetchFilters() {
        return fetchFilters;
    }
     ...
     */
    public void addFetchFilter(org.yaproxy.yap.spider.filters.FetchFilter filter) {
     ...
     */
    protected LinkedList<org.yaproxy.yap.spider.filters.ParseFilter> getParseFilters() {
        return parseFilters;
    }
     ...
     * @param filter the filter
     */
    public void addParseFilter(org.yaproxy.yap.spider.filters.ParseFilter filter) {
        log.debug("Loading parse filter: {}", filter.getClass().getSimpleName());
        parseFilters.add(filter);
    }

    protected void setDefaultParseFilter(org.yaproxy.yap.spider.filters.ParseFilter filter) {
        log.debug("Setting Default filter: {}", filter.getClass().getSimpleName());
        defaultParseFilter = filter;
    }

    protected org.yaproxy.yap.spider.filters.ParseFilter getDefaultParseFilter() {
    ...
    public void init() {
        ...
        for (org.yaproxy.yap.spider.parser.SpiderParser parser : parsers) {
    ...
    private String buildCanonicalResourceIdentifier(
            URI uri, org.yaproxy.yap.spider.parser.SpiderResourceFound resourceFound)
            throws URIException {
    ...
    @Override
    public void resourceFound(org.yaproxy.yap.spider.parser.SpiderResourceFound resourceFound) {
        ...
        for (org.yaproxy.yap.spider.filters.FetchFilter f : fetchFilters) {
            org.yaproxy.yap.spider.filters.FetchFilter.FetchStatus s = f.checkFilter(uriV);
            if (s != org.yaproxy.yap.spider.filters.FetchFilter.FetchStatus.VALID) {
        ...
        if (resourceFound.isShouldIgnore()) {
            ...
            spider.notifyListenersFoundURI(
                    ...
                    org.yaproxy.yap.spider.filters.FetchFilter.FetchStatus.VALID);
            return;
        }
     ...
     * @return the parsers
     */
    public List<org.yaproxy.yap.spider.parser.SpiderParser> getParsers() {
        return parsersUnmodifiableView;
    }

    public void addSpiderParser(org.yaproxy.yap.spider.parser.SpiderParser parser) {
