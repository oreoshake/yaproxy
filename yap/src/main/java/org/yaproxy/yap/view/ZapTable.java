...
import org.yaproxy.yap.utils.StickyScrollbarAdjustmentListener;
import org.yaproxy.yap.utils.TableExportAction;
...
public class YapTable extends JXTable {
    ...
    public YapTable(TableModel dataModel) {
        super(dataModel);
    ...
    private void init() {
        ...
        if (columnControl instanceof YapColumnControlButton) {
            YapColumnControlButton yapColumnControl = ((YapColumnControlButton) columnControl);
            autoScrollAction = createAutoScrollAction();
            if (autoScrollAction != null) {
                yapColumnControl.addAction(autoScrollAction);
            }
            TableExportAction<YapTable> exportAction = createTableExportAction();
            if (exportAction != null) {
                yapColumnControl.addAction(exportAction);
            }
            yapColumnControl.addAction(new ResetColumnsToDefaultAction(this));
            yapColumnControl.populatePopup();
        }
     ...
     *
     * <p>Called when customising the {@link YapColumnControlButton}.
     *
     ...
     */
    protected TableExportAction<YapTable> createTableExportAction() {
    ...
    protected JComponent createDefaultColumnControl() {
        return new YapColumnControlButton(this);
    }

    @SuppressWarnings("serial")
    protected static class YapColumnControlButton extends ColumnControlButton {

        private static final long serialVersionUID = -2888568545235496369L;

        private List<Action> customActions;

        public YapColumnControlButton(JXTable table) {
            super(table);
        }

        public YapColumnControlButton(JXTable table, Icon icon) {
            super(table, icon);
        }
    ...
    protected static class AutoScrollAction extends AbstractActionExt {
        ...
        private final YapTable table;

        public AutoScrollAction(YapTable table) {
        ...
        public AutoScrollAction(String label, Icon icon, YapTable table) {
            super(label, icon);
    ...
    protected static class ResetColumnsToDefaultAction extends AbstractActionExt {
        ...
        public ResetColumnsToDefaultAction(YapTable table) {
    ...
    private static class TableColumnConfiguration {
        ...
        public void apply(YapTable table) {
            // order matters here!
            this.rearrangeColumns(table);
            this.hideInvisibleColumns(table);
        }

        private void rearrangeColumns(YapTable table) {
        ...
        private TableColumn findColumnByIndex(YapTable table, int modelIndex) {
        ...
        private void hideInvisibleColumns(YapTable table) {
            List<javax.swing.table.TableColumn> columns =
                    ((DefaultTableColumnModelExt) table.columnModel).getColumns(true);
            for (int i = 0; i < columns.size(); i++) {
