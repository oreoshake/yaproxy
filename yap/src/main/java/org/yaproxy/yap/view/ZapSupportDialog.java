 ...
 * @since 2.7.0
 */
public class YapSupportDialog extends AbstractDialog {

    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = LogManager.getLogger(YapSupportDialog.class);

    private JPanel mainPanel = null;
    private YapSupportPanel supportPanel = null;
    private JButton btnOK = null;
    private JButton btnOpen = null;
    private JButton btnCopy = null;

    /**
     * Constructs an {@code YapSupportDialog} with no owner and not modal.
     *
     * @throws HeadlessException when {@code GraphicsEnvironment.isHeadless()} returns {@code true}
     */
    public YapSupportDialog() {
        this(null, true);
    }

    /**
     * Constructs an {@code YapSupportDialog} with the given owner and whether or not it's modal.
     *
     * @param owner the {@code Frame} from which the dialog is displayed
     * @param modal {@code true} if the dialogue should be modal, {@code false} otherwise
     * @throws HeadlessException when {@code GraphicsEnvironment.isHeadless()} returns {@code true}
     */
    public YapSupportDialog(Frame owner, boolean modal) {
        super(owner, modal);
    ...
    private YapSupportPanel getSupportPanel() {
        if (supportPanel == null) {
            supportPanel = new YapSupportPanel();
        }
        return supportPanel;
    }
    ...
    private JButton getBtnOK() {
        if (btnOK == null) {
            ...
            btnOK.addActionListener(e -> YapSupportDialog.this.dispose());
        }
        return btnOK;
    }
    ...
    private JButton getBtnOpen() {
        if (btnOpen == null) {
            ...
            btnOpen.addActionListener(
                    e -> {
                        try {
                            Desktop.getDesktop().open(new File(Constant.getYapHome()));
                        } catch (IOException e1) {
                            LOGGER.error(
                                    "An exception occurred while trying to have the OS open YAP's Home Directory.",
                                    e1);
                        }
                    });
        }
