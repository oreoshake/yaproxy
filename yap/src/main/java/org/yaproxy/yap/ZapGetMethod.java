 ...
 */
@Deprecated
public class YapGetMethod extends EntityEnclosingMethod {
    private static final Log LOG = LogFactory.getLog(YapGetMethod.class);
     ...
     */
    public YapGetMethod(String uri) {
        super(uri);
    }
    ...
    protected void readResponse(HttpState state, org.apache.commons.httpclient.HttpConnection conn)
        ...
        while (getStatusLine() == null) {
            ...
            if (status == 101) {
                ...
                if (conn instanceof YapHttpConnection) {
                    isUpgrade = true;
        ...
        if (conn instanceof YapHttpConnection) {
            YapHttpConnection yapConn = (YapHttpConnection) conn;
            upgradedSocket = yapConn.getSocket();
            inputStream = yapConn.getResponseInputStream();
        }
    ...
    protected void readResponseHeaders(
        ...
        Header[] headers =
                org.yaproxy.yap.network.YapHttpParser.parseHeaders(
