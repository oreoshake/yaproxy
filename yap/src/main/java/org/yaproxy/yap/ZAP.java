...
import org.yaproxy.yap.eventBus.EventBus;
import org.yaproxy.yap.eventBus.SimpleEventBus;

public class YAP {

    /**
     * YAP can be run in 4 different ways: cmdline: an inline process that exits when it completes
     ...
     * 'YAP as a Service' (this is 'work in progress')
     */
    ...
    private static final Logger LOGGER = LogManager.getLogger(YAP.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        YapBootstrap bootstrap = createYapBootstrap(cmdLine);
        try {
    ...
    private static YapBootstrap createYapBootstrap(CommandLine cmdLineArgs) {
        YapBootstrap bootstrap;
        if (cmdLineArgs.isGUI()) {
            YAP.processType = ProcessType.desktop;
            bootstrap = new GuiBootstrap(cmdLineArgs);
        } else if (cmdLineArgs.isDaemon()) {
            YAP.processType = ProcessType.daemon;
            bootstrap = new DaemonBootstrap(cmdLineArgs);
        } else {
            YAP.processType = ProcessType.cmdline;
