...
/**
 * An add-on entry of {@code YapVersions.xml} file.
 *
 * <p>It also reads:
 *
 * <ul>
 ...
 * @since 2.4.0
 */
public class YapVersionsAddOnEntry extends BaseYapAddOnXmlData {
    ...
    public YapVersionsAddOnEntry(HierarchicalConfiguration node) {
        super(node);
    }

    @Override
    protected void readAdditionalData(HierarchicalConfiguration yapAddOnData) {
        file = yapAddOnData.getString(FILE);
        size = yapAddOnData.getLong(SIZE);
        info = yapAddOnData.getString(INFO);
        date = yapAddOnData.getString(DATE, null);
        hash = yapAddOnData.getString(HASH, null);
    }
