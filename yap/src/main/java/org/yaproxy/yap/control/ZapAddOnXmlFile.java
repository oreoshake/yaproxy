 ...
 * @since 2.4.0
 */
public class YapAddOnXmlFile extends BaseYapAddOnXmlData {
    ...
    public YapAddOnXmlFile(InputStream is) throws IOException {
        super(is);
    }

    @Override
    protected void readAdditionalData(HierarchicalConfiguration yapAddOnXml) {
        ascanrules = getStrings(yapAddOnXml, ASCANRULES_ALL_ELEMENTS, ASCANRULE_ELEMENT);
        pscanrules = getStrings(yapAddOnXml, PSCANRULES_ALL_ELEMENTS, PSCANRULE_ELEMENT);
        files = getStrings(yapAddOnXml, FILES_ALL_ELEMENTS, FILE_ELEMENT);
        libs = getStrings(yapAddOnXml, LIBS_ALL_ELEMENTS, LIB_ELEMENT);

        bundleBaseName = yapAddOnXml.getString(BUNDLE_ELEMENT, "");
        bundlePrefix = yapAddOnXml.getString(BUNDLE_PREFIX_ATT, "");
        helpSetBaseName = yapAddOnXml.getString(HELPSET_ELEMENT, "");
        helpSetLocaleToken = yapAddOnXml.getString(HELPSET_LOCALE_TOKEN_ATT, "");
    }
