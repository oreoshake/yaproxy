...
import org.yaproxy.yap.authentication.AbstractAuthenticationMethodOptionsPanel;
import org.yaproxy.yap.authentication.AuthenticationIndicatorsPanel;
import org.yaproxy.yap.authentication.AuthenticationMethod;
import org.yaproxy.yap.authentication.AuthenticationMethod.AuthCheckingStrategy;
import org.yaproxy.yap.authentication.AuthenticationMethod.AuthPollFrequencyUnits;
import org.yaproxy.yap.authentication.AuthenticationMethodType;
...
import org.yaproxy.yap.view.NodeSelectDialog;
...
public class ContextAuthenticationPanel extends AbstractContextPropertiesPanel {
    ...
    private YapTextField pollUrlField = null;
    private YapTextField pollDataField = null;
    private YapTextArea pollHeadersField = null;
    private YapNumberSpinner pollFrequency = null;

    private YapTextField loggedInIndicatorRegexField = null;
    private YapTextField loggedOutIndicatorRegexField = null;
    ...
    private void initialize() {
        ...
        panel.add(new YapHtmlLabel(LABEL_DESCRIPTION), LayoutHelper.getGBC(0, 0, 1, 1.0D));
    ...
    private void changeMethodConfigPanel(AuthenticationMethodType newMethodType) {
        ...
        if (newMethodType.hasOptionsPanel()) {
            ...
            getConfigContainerPanel()
                    .add(new YapHtmlLabel(LABEL_CONFIG_NOT_NEEDED), BorderLayout.CENTER);
        }
    ...
    private YapNumberSpinner getPollFrequencySpinner() {
    ...
    private YapTextField getPollUrlField() {
        if (pollUrlField == null) {
            pollUrlField = new YapTextField();
        }
        return pollUrlField;
    }

    private YapTextField getPollDataField() {
        if (pollDataField == null) {
            pollDataField = new YapTextField();
        }
        return pollDataField;
    }

    private YapTextArea getPollHeadersField() {
        if (pollHeadersField == null) {
            pollHeadersField = new YapTextArea(2, 0);
        }
        return pollHeadersField;
    }

    private YapTextField getLoggedInIndicatorRegexField() {
        if (loggedInIndicatorRegexField == null) loggedInIndicatorRegexField = new YapTextField();
        return loggedInIndicatorRegexField;
    }

    private YapTextField getLoggedOutIndicatorRegexField() {
        if (loggedOutIndicatorRegexField == null) loggedOutIndicatorRegexField = new YapTextField();
