...
import org.yaproxy.yap.extension.api.ApiResponseList;
...
public class SpiderAPI extends ApiImplementor {
        ...
        switch (name) {
            ...
            case ACTION_ADD_DOMAIN_ALWAYS_IN_SCOPE:
                try {
                    ...
                    org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher domainAlwaysInScope;
                    if (getParam(params, PARAM_IS_REGEX, false)) {
                        domainAlwaysInScope =
                                new org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher(
                                        org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher
                                                .createPattern(value));
                    } else {
                        domainAlwaysInScope =
                                new org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher(value);
                    }
                    domainAlwaysInScope.setEnabled(getParam(params, PARAM_IS_ENABLED, true));

                    List<org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher> domainsAlwaysInScope =
            ...
            case ACTION_MODIFY_DOMAIN_ALWAYS_IN_SCOPE:
                try {
                    ...
                    org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher oldDomain =
                    ...
                    org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher newDomain;
                    if (getParam(params, PARAM_IS_REGEX, oldDomain.isRegex())) {
                        ...
                        newDomain = new org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher(value);
                    }
    ...
    private void setDomainsAlwaysInScopeEnabled(boolean enabled) {
        ...
        for (org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher x :
                extension.getSpiderParam().getDomainsAlwaysInScope()) {
    ...
    private int scanURL(
        ...
        if (startURI != null) {
            objs.add(startURI);
            if (subtreeOnly) {
                objs.add(new org.yaproxy.yap.spider.filters.HttpPrefixFetchFilter(startURI));
            }
        }

        if (maxChildren > 0) {
            // Add the filters to filter on maximum number of children
            org.yaproxy.yap.spider.filters.MaxChildrenFetchFilter maxChildrenFetchFilter =
                    new org.yaproxy.yap.spider.filters.MaxChildrenFetchFilter();
            maxChildrenFetchFilter.setMaxChildren(maxChildren);
            maxChildrenFetchFilter.setModel(extension.getModel());

            org.yaproxy.yap.spider.filters.MaxChildrenParseFilter maxChildrenParseFilter =
                    new org.yaproxy.yap.spider.filters.MaxChildrenParseFilter(
    ...
    private ApiResponse domainMatchersToApiResponseList(
            String name,
            List<org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher> domains,
            boolean excludeDisabled) {
        ApiResponseList apiResponse = new ApiResponseList(name);
        for (int i = 0; i < domains.size(); i++) {
            org.yaproxy.yap.spider.DomainAlwaysInScopeMatcher domain = domains.get(i);
