 ...
 * @see YapTocMerger.ForceMergeRequirement
 */
// Note: This class contains copied (verbatim) code from the base class UniteAppendMerge.
public class YapTocMerger extends UniteAppendMerge {

    private static final String DEFAULT_MERGE_TYPE = YapTocMerger.class.getCanonicalName();
    ...
    static {
        ...
        // (hard coded) names.
        tempMap.put("toplevelitem", new ForceMergeRequirement(1, "YAP User Guide"));
        tempMap.put(ADDONS_TOC_ID, new ForceMergeRequirement(2, "Add Ons"));
        TOC_IDS_FORCE_MERGE_MAP = Collections.unmodifiableMap(tempMap);
    }

    public YapTocMerger(NavigatorView master, NavigatorView slave) {
        super(master, slave);
    }
    ...
    // the method
    // YapTocMerger#mergeNodes(TreeNode, TreeNode) instead of UniteAppendMerge#mergeNodes(TreeNode,
    // TreeNode).
    @Override
    ...
    private static String getTOCID(DefaultMutableTreeNode node) {
        ...
        if (treeItem != null && (treeItem instanceof YapTocItem)) {
            return ((YapTocItem) treeItem).getTocId();
        }
        return null;
    }
