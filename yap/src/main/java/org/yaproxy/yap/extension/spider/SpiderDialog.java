...
public class SpiderDialog extends StandardFieldsDialog {
    ...
    private org.yaproxy.yap.spider.SpiderParam spiderParam = null;
    ...
    private org.yaproxy.yap.spider.SpiderParam getSpiderParam() {
        if (spiderParam == null) {
            // First time in clone the global options, after that keep the last ones the user set
            spiderParam = (org.yaproxy.yap.spider.SpiderParam) extension.getSpiderParam().clone();
        }
        return spiderParam;
    }
        ...
        if (startUri != null) {
            ...
            if (getBoolValue(FIELD_SUBTREE_ONLY)) {
                contextSpecificObjects.add(
                        new org.yaproxy.yap.spider.filters.HttpPrefixFetchFilter(startUri));
            }
        }
