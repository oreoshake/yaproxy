...
import org.yaproxy.yap.extension.api.API.RequestType;
...
public abstract class ApiImplementor {
    ...
    /**
     * Tells whether or not the given {@code method} should be ignored, thus not included in the YAP
     * API.
     *
     * <p>Checks if the given {@code method} has been annotated with {@code YapApiIgnore} or if it's
    ...
    private static boolean isIgnored(Method method) {
        return method.getAnnotation(YapApiIgnore.class) != null
    ...
    private void setApiOptionDeprecated(ApiElement apiOption, boolean deprecated) {
        if (deprecated) {
            ...
            if (Constant.messages != null) {
                // Add a custom message when running from YAP.
     ...
     * @since 2.9.0
     * @see org.yaproxy.yap.utils.I18N#getString(String)
     */
