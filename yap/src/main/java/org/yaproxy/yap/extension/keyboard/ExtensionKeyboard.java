 ...
 */
package org.yaproxy.yap.extension.keyboard;
...
public class ExtensionKeyboard extends ExtensionAdaptor {
     ...
     *
     * @see #getDefaultAccelerator(YapMenuItem)
     */
    ...
    public void registerMenuItem(YapMenuItem zme) {
        ...
        if (identifier != null) {
            ...
            LOGGER.warn("YapMenuItem \"{}\" has a null identifier.", zme.getName());
        }
    }
     ...
     */
    private void validateDefaultAccelerator(YapMenuItem zme) {
    ...
    private void initAllMenuItems(JMenu menu) {
        for (Component c : menu.getMenuComponents()) {
            if (c instanceof YapMenuItem) {
                this.registerMenuItem((YapMenuItem) c);
    ...
    private void setConfiguredAccelerator(YapMenuItem menuItem) {
    ...
    private void addAllMenuItems(List<KeyboardShortcut> kss, JMenu menu, boolean reset) {
        for (Component c : menu.getMenuComponents()) {
            if (c instanceof YapMenuItem) {
                kss.add(menuToShortcut((YapMenuItem) c, reset));
    ...
    private KeyboardShortcut menuToShortcut(YapMenuItem menuItem, boolean reset) {
        if (reset) {
     ...
     */
    private KeyStroke getDefaultAccelerator(YapMenuItem menuItem) {
