 ...
 *
 * It was copied because YAP depends (and uses) Commons HttpClient which is not compatible with, the newer version,
 * HttpComponents Client.
 *
 * It's adapted to work with Commons HttpClient.
 * Changes:
 *  - Renamed the class to YapNTLMEngineImpl to not clash with the name NTLMEngineImpl;
 *  - Moved to "org.yaproxy.yap.network" (instead of keeping it in "org.apache.http.impl.auth");
...
final class YapNTLMEngineImpl {
    ...
    static class NTLMMessage {
        ...
        protected int readUShort(final int position) throws AuthenticationException {
            return YapNTLMEngineImpl.readUShort(messageContents, position);
        }

        /** Read a ulong from a position within the message buffer */
        protected int readULong(final int position) throws AuthenticationException {
            return YapNTLMEngineImpl.readULong(messageContents, position);
        }

        /** Read a security buffer from a position within the message buffer */
        protected byte[] readSecurityBuffer(final int position) throws AuthenticationException {
            return YapNTLMEngineImpl.readSecurityBuffer(messageContents, position);
        }
