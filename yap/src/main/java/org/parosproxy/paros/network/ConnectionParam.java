 ...
 */
// YAP: 2011/08/03 Cope with unexpected values in config file
// YAP: 2012/04/23 Added @Override annotation to the appropriate method and removed
// unnecessary cast.
...
// YAP: 2013/01/04 Added portsForSslTunneling parameter with method
...
// YAP: 2014/03/23 Issue 1100: Annotate option methods that shouldn't be exposed in the YAP API
// YAP: 2041/08/14 Issue 1305: Outgoing proxy is disabled when updating from old versions
// YAP: 2016/08/08 Issue 2742: Allow for override/customization of Java's "networkaddress.cache.ttl"
// value
// YAP: 2017/01/11 Exclude some options from the API (manually handled to return correct values).
// YAP: 2017/04/14 Validate that the SSL/TLS versions persisted can be set/used.
// YAP: 2017/05/02 Added option key to enable / disable HTTP State
// YAP: 2017/05/15 Ensure HttpState is non-null when HTTP State is enabled.
// YAP: 2017/06/19 Do not allow to set negative timeout values and expose the default value.
// YAP: 2017/09/26 Use helper methods to read the configurations.
// YAP: 2018/02/14 Remove unnecessary boxing / unboxing
// YAP: 2018/08/10 Set the default user agent to HttpRequestHeader (Issue 4846).
// YAP: 2019/06/01 Normalise line endings.
// YAP: 2019/06/05 Normalise format/style.
// YAP: 2020/01/02 Updated default user agent
// YAP: 2020/04/20 Allow to configure the SOCKS proxy (Issue 29).
// YAP: 2020/11/26 Use Log4j 2 classes for logging.
// YAP: 2021/10/06 Updated default user agent
// YAP: 2022/02/02 Removed getProxyChainSkipName() and setProxyChainSkipName(String)
// YAP: 2022/02/08 Use isEmpty where applicable.
// YAP: 2022/05/04 Deprecate single cookie request header option.
...
public class ConnectionParam extends AbstractParam {
    ...
    // YAP: Added prompt option and timeout
     ...
     * @since 2.10.0
     */
    public static final org.yaproxy.yap.network.SocksProxy DEFAULT_SOCKS_PROXY =
            new org.yaproxy.yap.network.SocksProxy("localhost", 1080);
    ...
    private org.yaproxy.yap.network.SocksProxy socksProxy = DEFAULT_SOCKS_PROXY;
    ...
    // YAP: Added setProxyChainPassword(String, boolean) method
    ...
    private void parseSocksProxyOptions() {
        ...
        socksProxy =
                new org.yaproxy.yap.network.SocksProxy(
                        host,
                        port,
                        org.yaproxy.yap.network.SocksProxy.Version.from(version),
                        useDns);
        if (useSocksProxy) {
            apply(socksProxy);
        }
     ...
     */
    private void apply(org.yaproxy.yap.network.SocksProxy socksProxy) {
        String host = "";
        String port = "";
        String version = "";
        if (useSocksProxy) {
     ...
     *
     * <p>The names should not be resolved when YAP is configured to use a SOCKSv5 proxy and rely on
     * it for resolution.
     *
    ...
    public boolean shouldResolveRemoteHostname(String hostname) {
        if (!useSocksProxy
                || !socksProxy.isUseDns()
                || socksProxy.getVersion() != org.yaproxy.yap.network.SocksProxy.Version.SOCKS5) {
            return true;
        }
     ...
     * @since 2.10.0
     * @see #isUseSocksProxy()
     * @see #setSocksProxy(org.yaproxy.yap.network.SocksProxy)
     */
     ...
     */
    @YapApiIgnore
    public org.yaproxy.yap.network.SocksProxy getSocksProxy() {
        return socksProxy;
    }
     ...
     * @see #setUseSocksProxy(boolean)
     */
    public void setSocksProxy(org.yaproxy.yap.network.SocksProxy socksProxy) {
