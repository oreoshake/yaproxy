 ...
 */
// YAP: 2012/01/12 Changed the method createRequestMethod to always use CRLF
// YAP: 2012/03/15 Changed to use the class StringBuilder instead of StringBuffer
// YAP: 2012/05/04 Changed to use the class YapGetMethod instead of
// org.apache.commons.httpclient.methods.GetMethod
// YAP: 2013/01/23 Clean up of exception handling/logging.
// YAP: 2013/06/17 Issue 687: Change HTTP response header parser to be less strict
// YAP: 2013/07/10 Issue 721: Non POST and PUT requests receive a 504 when server expects a request
// body
// YAP: 2016/05/16 Throw exception if failed to set the request URI
// YAP: 2016/10/13 Include URI in exception's message
// YAP: 2017/04/20 Remove unused code
// YAP: 2017/06/19 Allow to create a method with custom parameters.
...
public class HttpMethodHelper {
    ...
    public HttpMethod createRequestMethodNew(HttpRequestHeader header, HttpBody body)
        ...
        // set various headers
        int pos = 0;
        // YAP: FindBugs fix - always initialise pattern
    ...
    HttpMethod createRequestMethod(HttpRequestHeader header, HttpBody body, HttpMethodParams params)
        ...
        if (method.equalsIgnoreCase(GET)) {
            // YAP: avoid discarding HTTP status code 101 that is used for WebSocket upgrade
            httpMethod = new org.yaproxy.yap.YapGetMethod();
        } else if (method.equalsIgnoreCase(POST)) {
            httpMethod = new org.yaproxy.yap.network.YapPostMethod();
        } else if (method.equalsIgnoreCase(DELETE)) {
            httpMethod = new org.yaproxy.yap.network.YapDeleteMethod();
        } else if (method.equalsIgnoreCase(PUT)) {
            httpMethod = new org.yaproxy.yap.network.YapPutMethod();
        } else if (method.equalsIgnoreCase(HEAD)) {
            httpMethod = new org.yaproxy.yap.network.YapHeadMethod();
        } else if (method.equalsIgnoreCase(OPTIONS)) {
            httpMethod = new org.yaproxy.yap.network.YapOptionsMethod();
        } else if (method.equalsIgnoreCase(TRACE)) {
            httpMethod = new org.yaproxy.yap.network.YapTraceMethod(uri.toString());
        } else {
            httpMethod = new GenericMethod(method);
        }
        ...
        // set various headers
        int pos = 0;
        // YAP: changed to always use CRLF, like the HttpHeader
        Pattern pattern = patternCRLF;
        String delimiter = header.getLineDelimiter();

        // YAP: Shouldn't happen as the HttpHeader always uses CRLF
