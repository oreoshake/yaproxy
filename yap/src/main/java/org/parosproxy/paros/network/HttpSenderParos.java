 ...
 */
// YAP: 2022/06/03 Move implementation from HttpSender.
...
public class HttpSenderParos implements HttpSenderImpl<HttpSenderContextParos> {
    ...
    static {
        ...
        AuthPolicy.registerAuthScheme(AuthPolicy.NTLM, org.yaproxy.yap.network.YapNTLMScheme.class);
        CookiePolicy.registerCookieSpec(
                CookiePolicy.DEFAULT, org.yaproxy.yap.network.YapCookieSpec.class);
        CookiePolicy.registerCookieSpec(
                CookiePolicy.BROWSER_COMPATIBILITY, org.yaproxy.yap.network.YapCookieSpec.class);
    }
    ...
    int executeMethodImpl(HttpSenderContextParos ctx, HttpMethod method, HttpState state)
        ...
        if (isConnectionUpgrade(method)) {
            requestClient = new HttpClient(new org.yaproxy.yap.YapHttpConnectionManager());
        } else {
        ...
        // YAP: Check if a custom state is being used
        if (state != null) {
    ...
    private void send(
        ...
        try {
            ...
            // YAP: set method to retrieve upgraded channel later
            if (method instanceof org.yaproxy.yap.YapGetMethod) {
