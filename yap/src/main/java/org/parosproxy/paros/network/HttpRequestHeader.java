 ...
 */
// YAP: 2012:02/01 Changed getHostPort() to return proper port number even if it
// is not explicitly specified in URI
// YAP: 2011/08/04 Changed to support Logging
// YAP: 2011/10/29 Log errors
// YAP: 2011/11/03 Changed isImage() to prevent a NullPointerException when the path doesn't exist
// YAP: 2011/12/09 Changed HttpRequestHeader(String method, URI uri, String version) to add
...
// YAP: 2012/03/15 Changed to use the class StringBuilder instead of StringBuffer. Reworked some
// methods.
...
// YAP: 2012/06/24 Added new method of getting cookies from the request header.
// YAP: 2012/10/08 Issue 361: getHostPort on HttpRequestHeader for HTTPS CONNECT
// requests returns the wrong port
// YAP: 2013/01/23 Clean up of exception handling/logging.
// YAP: 2013/03/08 Improved parse error reporting
// YAP: 2013/04/14 Issue 596: Rename the method HttpRequestHeader.getSecure to isSecure
// YAP: 2013/05/02 Re-arranged all modifiers into Java coding standard order
// YAP: 2013/12/09 Set Content-type only in case of POST or PUT HTTP methods
// YAP: 2015/08/07 Issue 1768: Update to use a more recent default user agent
// YAP: 2016/06/17 Remove redundant initialisations of instance variables
// YAP: 2016/09/26 JavaDoc tweaks
// YAP: 2017/02/23 Issue 3227: Limit API access to permitted IP addresses
// YAP: 2017/04/24 Added more HTTP methods
// YAP: 2017/10/19 Skip parsing of empty Cookie headers.
// YAP: 2017/11/22 Address a NPE in isImage().
// YAP: 2018/01/10 Tweak how cookie header is reconstructed from HtmlParameter(s).
// YAP: 2018/02/06 Make the upper case changes locale independent (Issue 4327).
// YAP: 2018/08/10 Allow to set the user agent used by default request headers (Issue 4846).
// YAP: 2018/11/16 Add Accept header.
// YAP: 2019/01/25 Add Origin header.
// YAP: 2019/03/06 Log or include the malformed data in the exception message.
// YAP: 2019/03/19 Changed the parse method to only parse the authority on CONNECT requests
...
// YAP: 2020/11/10 Add convenience method isCss(), refactor isImage() to use new private method
// isSpecificType(Pattern).
// YAP: 2020/11/26 Use Log4j 2 classes for logging.
// YAP: 2021/05/10 Use authority for CONNECT requests.
// YAP: 2021/07/16 Issue 6691: Do not add zero Content-Length by default in GET requests
// YAP: 2021/07/19 Include SVG in isImage().
// YAP: 2022/09/12 Allow arbitrary HTTP versions.
// YAP: 2022/09/21 Use format specifiers instead of concatenation when logging.
// YAP: 2022/11/22 Lower case the HTTP field names for compatibility with HTTP/2.
...
public class HttpRequestHeader extends HttpHeader {
    ...
    public HttpRequestHeader(String method, URI uri, String version)
        ...
        // YAP: added the Cache-Control header field to comply with HTTP/1.1
        if (version.equalsIgnoreCase(HTTP11)) {
            setHeader(CACHE_CONTROL, "no-cache");
        }

        // YAP: set content type x-www-urlencoded only if it's a POST or a PUT
    ...
    public String getHostName() {
        ...
        try {
            // YAP: fixed cases, where host name is null
    ...
    private boolean isSpecificType(Pattern pattern) {
        ...
        try {
            // YAP: prevents a NullPointerException when no path exists
    ...
    // YAP: Added method for working directly with HttpCookie
    /**
