 ...
 */
// YAP: 2012/03/15 Removed the options to change the display of the ManualRequestEditorDialog,
// now they are changed dynamically.
// YAP: 2012/04/25 Added @Override annotation to all appropriate methods.
// YAP: 2013/12/13 Added support for a new option 'show tab names'.
...
// YAP: 2017/01/09 Remove method no longer needed.
// YAP: 2018/02/14 Add option for ResponsePanelPosition.TAB_SIDE_BY_SIDE (Issue 4331).
// YAP: 2018/02/27 Add option for Look And Feel and a scrollbar.
// YAP: 2018/03/01 Remove the name from a panel and use BorderLayout.
...
// YAP: 2020/02/24 Use LookAndFeelInfo when setting the look and feel option.
// YAP: 2020/03/25 Remove hardcoded colour in titled border (Issue 5542).
// YAP: 2020/12/03 Add constants for indexes of possible break buttons locations
...
// YAP: 2022/02/25 Remove options no longer in use.
...
import org.yaproxy.yap.extension.brk.BreakpointsParam;
...
// YAP: 2011: added more configuration options
...
public class OptionsViewPanel extends AbstractParamPanel {
    ...
    private Map<FontUtils.FontType, YapNumberSpinner> fontSizes =
    ...
    private Map<FontUtils.FontType, String> fontTypeLabels =
            new EnumMap<>(FontUtils.FontType.class);
    private YapNumberSpinner iconSize;
    ...
    private YapNumberSpinner getIconSize() {
        if (iconSize == null) {
            iconSize = new YapNumberSpinner(0, 16, 100);
    ...
    private YapNumberSpinner initFontSize(FontUtils.FontType fontType) {
        YapNumberSpinner fontSize;
        fontSize = new YapNumberSpinner(-1, 8, 100);
    ...
    private YapNumberSpinner getFontSize(FontUtils.FontType fontType) {
