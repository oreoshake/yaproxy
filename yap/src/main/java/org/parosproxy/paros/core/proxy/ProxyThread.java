 ...
 */
// YAP: 2011/05/09 Support for API
// YAP: 2011/05/15 Support for exclusions
// YAP: 2012/03/15 Removed unnecessary castings from methods notifyListenerRequestSend,
...
// YAP: 2012/05/11 Do not close connections in final clause of run() method,
// if boolean attribute keepSocketOpen is set to true.
// YAP: 2012/08/07 Issue 342 Support the HttpSenderListener
// YAP: 2012/11/04 Issue 408: Add support to encoding transformations, added an
// option to control whether the "Accept-Encoding" request-header field is
// modified/removed or not.
// YAP: 2012/12/27 Added support for PersistentConnectionListener.
// YAP: 2013/01/04 Do beginSSL() on HTTP CONNECT only if port requires so.
// YAP: 2013/03/03 Issue 547: Deprecate unused classes and methods
// YAP: 2013/04/11 Issue 621: Handle requests to the proxy URL
// YAP: 2013/04/14 Issue 622: Local proxy unable to correctly detect requests to itself
// YAP: 2013/06/17 Issue 686: Log HttpException (as error) in the ProxyThread
// YAP: 2013/12/13 Issue 939: YAP should accept SSL connections on non-standard ports automatically
// YAP: 2014/03/06 Issue 1063: Add option to decode all gzipped content
// YAP: 2014/03/23 Tidy up, extracted a method that writes an HTTP response and moved the
// code responsible to decode a GZIP response to a method
// YAP: 2014/03/23 Fixed an issue with ProxyThread that happened when the proxy was set to listen on
// any address in which case the requests to the proxy itself were not correctly detected.
// YAP: 2014/03/23 Issue 122: ProxyThread logging timeout readings with incorrect message (URL)
// YAP: 2014/03/23 Issue 585: Proxy - "502 Bad Gateway" errors responded as "504 Gateway Timeout"
// YAP: 2014/03/23 Issue 969: Proxy - Do not include the response body when answering unsuccessful
// HEAD requests
// YAP: 2014/03/23 Issue 1017: Proxy set to 0.0.0.0 causes incorrect PAC file to be generated
// YAP: 2014/03/23 Issue 1022: Proxy - Allow to override a proxied message
// YAP: 2014/04/17 Issue 1156: Proxy gzip decoder doesn't update content length in response headers
// YAP: 2014/05/01 Issue 1156: Proxy gzip decoder removes newlines in decoded response
// YAP: 2014/05/01 Issue 1168: Add support for deflate encoded responses
// YAP: 2015/01/04 Issue 1334: YAP does not handle API requests on reused connections
// YAP: 2015/02/24 Issue 1540: Allow proxy scripts to fake responses
// YAP: 2015/07/17 Show stack trace of the exceptions on proxy errors
// YAP: 2016/03/18 Issue 2318: YAP Error [java.net.SocketTimeoutException]: Read timed out when
// running on AWS EC2 instance
// YAP: 2016/04/13 Notify of timeouts when reading a response
// YAP: 2016/04/14 Delay the write of response to not attempt to write a response again when
// handling IOException
// YAP: 2016/04/29 Adjust exception logging levels and log when timeouts happen
// YAP: 2016/05/30 Issue 2494: YAP Proxy is not showing the HTTP CONNECT Request in history tab
// YAP: 2016/06/13 Remove all unsupported encodings (instead of just some)
// YAP: 2016/09/22 JavaDoc tweaks
// YAP: 2016/11/28 Correct proxy errors' Content-Length value.
// YAP: 2016/12/07 Allow to extend the ProxyThread and use a custom HttpSender
// YAP: 2016/12/23 Make SocketTimeoutException less verbose for general use
// YAP: 2017/02/08 Differentiate client read timeout after CONNECT, from server read timeout.
// YAP: 2017/02/08 Change CONNECT response to contain just the status line, helps Android emulator
// consume the response.
// YAP: 2017/02/20 Issue 2699: Make SSLException handling more user friendly
// YAP: 2017/02/23  Issue 3227: Limit API access to permitted IP addresses
// YAP: 2017/03/15 Disable API by default
// YAP: 2017/03/26 Check the public address when behind NAT.
// YAP: 2017/06/12 Do not notify listeners when request is excluded.
// YAP: 2017/09/22 Check if first message received is a SSL/TLS handshake and tweak exception
// message.
// YAP: 2017/10/02 Improve error handling when checking if SSL/TLS handshake.
// YAP: 2018/01/29 Fix API issues with pconn connections
// YAP: 2019/04/08 Issue 5304: Check for UnknownHostException and include appropriate message if
...
// YAP: 2020/12/09 Rely on the content encodings from the body to decode.
// YAP: 2022/02/09 Deprecate the class.
// YAP: 2022/05/20 Address deprecation warnings with ConnectionParam.
// YAP: 2022/06/05 Address deprecation warnings with HttpException.
// YAP: 2022/06/07 Address deprecation warnings with YapGetMethod.
// YAP: 2022/09/21 Use format specifiers instead of concatenation when logging.
// YAP: 2022/09/26 Remove usage of org.ice4j classes.
...
public class ProxyThread implements Runnable {
    ...
    // YAP: New attribute to allow for skipping disconnect
    ...
    protected ProxyThread(ProxyServer server, Socket socket, HttpSender httpSender) {
        ...
        thread = new Thread(this, "YAP-ProxyThread-" + id++); // YAP: Set the name of the thread.
    ...
    private void beginSSL(String targethost) throws IOException {
        // YAP: added parameter 'targethost'
        try {
        ...
        try {
            ...
            if (firstHeader.getMethod().equalsIgnoreCase(HttpRequestHeader.CONNECT)) {
                ...
                try {
                    ...
                    // May we can redirect to some kind of YAP custom error page.

                    final HttpMessage errmsg = new HttpMessage(firstHeader);
                    setErrorResponse(errmsg, BAD_GATEWAY_RESPONSE_STATUS, e, "YAP SSL Error");
            ...
            // YAP: do only close if flag is false
            if (!keepSocketOpen) {
                disconnect();
            }
        }
    ...
    private void setErrorResponse(HttpMessage msg, String responseStatus, Exception cause)
        ...
        setErrorResponse(msg, responseStatus, cause, "YAP Error");
    }
    ...
    protected void processHttp(HttpRequestHeader requestHeader, boolean isSecure)
        ...
        HttpRequestBody reqBody =
                null; // YAP: Replaced the class HttpBody with the class HttpRequestBody.
        ...
        do {
            ...
            if (parentServer.isEnableApi()) {
                ...
                if (msg != null) {
                    ...
                    org.yaproxy.yap.YapGetMethod method = new org.yaproxy.yap.YapGetMethod();
            ...
            if (msg.getRequestHeader().getContentLength() > 0) {
                reqBody =
                        httpIn.readRequestBody(
                                requestHeader); // YAP: Changed to call the method readRequestBody.
            ...
            org.yaproxy.yap.YapGetMethod method =
                    (org.yaproxy.yap.YapGetMethod) msg.getUserObject();
            keepSocketOpen = notifyPersistentConnectionListener(msg, inSocket, method);
            if (keepSocketOpen) {
    ...
    private boolean notifyPersistentConnectionListener(
            HttpMessage httpMessage, Socket inSocket, org.yaproxy.yap.YapGetMethod method) {
