 ...
 */
// YAP: 2011/05/15 Improved error logging
// YAP: 2012/02/18 Rationalised session handling
// YAP: 2012/04/23 Added @Override annotation to all appropriate methods.
// YAP: 2012/06/11 Changed to call the method Control.shutdown(boolean) with the
// parameter set as true.
// YAP: 2012/06/19 Changed the method sessionOpened(File,Exception) to not call
// the method ExtensionLoader.sessionChangedAllPlugin, now it's done in the
// class Control.
// YAP: 2012/07/02 Changed to use the new database compact option in the method
// exit().
// YAP: 2012/07/23 Removed parameter from View.getSessionDialog call.
// YAP: 2012/12/06 Issue 428: Moved exit code to control to support the marketplace
...
// YAP: 2013/08/05 Proper call for starting Session Properties dialog
...
// YAP: 2015/04/02 Issue 321: Support multiple databases
// YAP: 2015/12/14 Log exception and internationalise error message
// YAP: 2016/10/26 Issue 1952: Do not allow Contexts with same name
// YAP: 2017/02/25 Issue 2618: Let the user select the name for snapshots
// YAP: 2017/06/01 Issue 3555: setTitle() functionality moved in order to ensure consistent
// application
// YAP: 2017/06/20 Inform of active actions before changing the session.
// YAP: 2017/08/31 Use helper method I18N.getString(String, Object...).
// YAP: 2017/11/22 Do not allow to snapshot the session with active actions (Issue 3711).
// YAP: 2017/12/15 Confirm when overwriting session file (Issue 4153).
// YAP: 2018/01/01 Prevent the selection of the current session on save/snapshot.
...
// YAP: 2022/08/05 Address warns with Java 18.
// YAP: 2022/09/21 Use format specifiers instead of concatenation when logging.
// YAP: 2023/01/10 Tidy up logger.
// YAP: 2023/05/21 Handle duplicate context names when importing in a more userfriendly way (Issue
// 7421).
...
import org.yaproxy.yap.view.ContextExportDialog;
import org.yaproxy.yap.view.PersistSessionDialog;
import org.yaproxy.yap.view.SessionTableSelectDialog;
...
public class MenuFileControl implements SessionListener {
    ...
    private boolean informStopActiveActions() {
        ...
        if (!activeActions.isEmpty()) {
            ...
            if (view.showConfirmDialog(new YapHtmlLabel(message)) != JOptionPane.OK_OPTION) {
                return false;
            }
        }
        return true;
    ...
    private String getTimestampFilename() {
        File dir = new File(Constant.getYapHome(), "sessions");
        if (!dir.exists()) {
            if (!dir.mkdirs()) {
                return null;
            }
    ...
    private void openFileBasedSession() {
        ...
        chooser.setFileHidingEnabled(
                false); // By default YAP on linux puts timestamped sessions under a 'dot' directory
        File file = null;
    ...
    public void saveSession() {
        ...
        try {
            waitMessageDialog =
                    view.getWaitMessageDialog(
                            Constant.messages.getString("menu.file.savingSession")); // YAP: i18n
            control.saveSession(session.getFileName(), this);
            LOGGER.info("saving session file {}", session.getFileName());
            // YAP: If the save is quick the dialog can already be null here
            if (waitMessageDialog != null) {
                waitMessageDialog.setVisible(true);
            }
            ...
            view.showWarningDialog(
                    Constant.messages.getString("menu.file.savingSession.error")); // YAP: i18n
    ...
    public void saveAsSession() {
        ...
        // YAP: set session name as file name proposal
                ...
                waitMessageDialog =
                        view.getWaitMessageDialog(
                                Constant.messages.getString(
                                        "menu.file.savingSession")); // YAP: i18n
        ...
        if (rc == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                waitMessageDialog =
                        view.getWaitMessageDialog(
                                Constant.messages.getString(
                                        "menu.file.savingSnapshot")); // YAP: i18n
    ...
    public void properties() {
        // YAP: proper call of existing method
    ...
    public void sessionOpened(File file, Exception e) {
        if (e == null) {
            // YAP: Removed the statement that called the method
    ...
    public void sessionSnapshot(Exception e) {
        if (e != null) {
            view.showWarningDialog(
                    Constant.messages.getString("menu.file.snapshotSession.error")); // YAP: i18n
    ...
    public void importContext() {
        ...
        chooser.setFileFilter(
                new FileFilter() {
                    ...
                    public String getDescription() {
                        return Constant.messages.getString("file.format.yap.context");
                    }
                });
        ...
        if (rc == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                YapXmlConfiguration config = new YapXmlConfiguration(file);
    ...
    private static class SessionFileChooser extends WritableFileChooser {
        ...
        public static final FileFilter SESSION_FILE_FILTER =
                new FileFilter() {
                    ...
                    @Override
                    public String getDescription() {
                        return Constant.messages.getString("file.format.yap.session");
                    }
                };
