 ...
 */
// YAP: 2011/04/16 Support for running YAP as a daemon
// YAP: 2011/05/09 Support for API
// YAP: 2011/05/15 Support for exclusions
// YAP: 2011/10/29 Support for parameters
// YAP: 2011/11/20 Changed to use ExtensionFactory
// YAP: 2012/02/18 Rationalised session handling
// YAP: 2012/04/23 Changed the method shutdown(boolean) to save the configurations
...
// YAP: 2012/05/16 Added new initialization method plus ctor for testing purposes.
// YAP: 2012/06/25 Moved call of init() from ctor to singleton methods to
...
// YAP: 2012/11/01 Issue 411: Allow proxy port to be specified on the command line
// YAP: 2012/12/06 Issue 428: Added exit method to support the marketplace
// YAP: 2012/12/27 Hook new persistent connection listener.
// YAP: 2013/03/03 Issue 546: Remove all template Javadoc comments
// YAP: 2013/03/20 Issue 568: Allow extensions to run from the command line
// YAP: 2013/04/16 Issue 638: Persist and snapshot sessions instead of saving them
// YAP: 2013/08/28 Issue 695: Sites tree doesn't clear on new session created by API
// YAP: 2013/08/29 Issue 776: Allow add-ons to warn user if they're closing YAP with unsaved
// resources open
// YAP: 2013/09/16 Issue 791: Saved sessions are discarded on YAP's exit
// YAP: 2014/01/16 Issue 979: Sites and Alerts trees can get corrupted
// YAP: 2014/02/21 Issue 1043: Custom active scan dialog
// YAP: 2014/02/27 Issue 1055: Load extensions before plugins
// YAP: 2014/05/20 Issue 1114: core.newSession doesn't clear Sites
// YAP: 2014/05/20 Issue 1191: Cmdline session params have no effect
// YAP: 2014/09/22 Issue 1345: Support Attack mode
// YAP: 2014/11/19 Issue 1412: Manage scan policies
// YAP: 2015/01/29 Issue 1489: Version number in window title
...
// YAP: 2015/09/17 Issue 1914: Support multiple add-on directories
// YAP: 2015/11/04 Issue 1920: Report the host:port YAP is listening on in daemon mode, or exit if
// it cant
// YAP: 2016/03/23 Issue 2331: Custom Context Panels not show in existing contexts after
// installation of add-on
// YAP: 2016/04/22 Issue 2428: Memory leak on session creation/loading
// YAP: 2016/05/30 Issue 2494: YAP Proxy is not showing the HTTP CONNECT Request in history tab
// YAP: 2016/09/06 Hook OverrideMessageProxyListener into the Proxy
// YAP: 2016/10/06 Issue 2855: Added method to allow for testing when a model is required
// YAP: 2017/03/10 Reset proxy excluded URLs on new session
// YAP: 2017/03/13 Set global excluded URLs to the proxy when creating a new session or
// initialising.
// YAP: 2017/03/16 Allow to initialise Control without starting the Local Proxy.
...
// YAP: 2018/01/04 Do not notify extensions if failed to change the session.
// YAP: 2018/01/12 Save configurations as last shutdown action.
// YAP: 2019/03/14 Improve error handling on shutdown
// YAP: 2019/06/01 Normalise line endings.
// YAP: 2019/06/05 Normalise format/style.
// YAP: 2019/09/30 Reduce View singleton usage and replace null checks with hasView().
// YAP: 2019/12/16 Log path of new session.
// YAP: 2019/12/13 Enable prompting/suggesting a new port when there's a proxy port conflict (Issue
// 2016).
// YAP: 2020/11/23 Allow to initialise the singleton with an ExtensionLoader for tests.
// YAP: 2020/11/26 Use Log4j 2 classes for logging.
// YAP: 2021/05/14 Remove empty statement.
// YAP: 2021/09/13 Added setExitStatus.
// YAP: 2021/11/08 Validate if mandatory add-ons are present.
// YAP: 2022/02/09 No longer manage the proxy, deprecate related code.
// YAP: 2022/02/24 Remove code deprecated in 2.5.0
...
public class Control extends AbstractControl implements SessionListener {
    ...
    private Control(Model model, View view) {
        super(model, view);
        // YAP: moved call of init() to singleton methods
    }

    // YAP: Added constructor that will be used by initSingletonForTesting()
    private Control() {
        super(null, null);
    }
    ...
    public void exit(boolean noPrompt, final File openOnExit) {
        ...
        if (!noPrompt) {
            ...
            if (message != null
                    && view.showConfirmDialog(new YapHtmlLabel(message)) != JOptionPane.OK_OPTION) {
                return;
            }
        }
        ...
        Thread t =
                new Thread(
                        new Runnable() {
                            @Override
                            public void run() {
                                // YAP: Changed to use the option compact database.
                                try {
                                    control.shutdown(
                                            Model.getSingleton()
                                                    .getOptionsParam()
        ...
        if (hasView()) {
            WaitMessageDialog dialog =
                    view.getWaitMessageDialog(
                            Constant.messages.getString("menu.file.shuttingDown")); // YAP: i18n
            t.start();
    ...
    /**
     * Sets the non zero value YAP will exit cleanly with. YAP may still exit with a non zero value
     * if a serious error occurs. This will work however YAP is run but it makes more sense if YAP
     * is run in cmdline mode. Any add-on can set an exit status so attempts to reset the status to
     * zero will be rejected.
     *
     * @param exitStatus the non zero value YAP will exit it with
    ...
    // YAP: Added method to allow for testing
    public static void initSingletonForTesting() {
        control = new Control();
    }

    // YAP: Added method to allow for testing when a model is required
