 ...
 */
// YAP: 2011/05/27 Catch any exception when loading the config file
// YAP: 2011/11/15 Changed to use YapXmlConfiguration, to enforce the same character encoding when
...
// YAP: 2017/03/26 Obtain configs in the order specified
// YAP: 2017/06/02 Add helper methods.
...
// YAP: 2023/07/06 Add method to read enum values.
...
public abstract class AbstractParam implements Cloneable {
    ...
    public void load(String filePath, ControlOverrides overrides) {
        try {
            config = new YapXmlConfiguration(filePath);
            if (overrides != null) {
