 ...
 */
// YAP: 2012/05/31 Changed to log the exceptions and one message in the method
// getNewConnection().
// YAP: 2012/08/16 SHUTDOWN COMPACT old databases.
...
// YAP: 2022/09/21 Remove unnecessary debug level checking.
...
public class ParosDatabaseServer implements DatabaseServer {
    ...
    private void start(String dbname) throws ClassNotFoundException, Exception {
        ...
        // YAP: Check if old database should be compacted
        ...
        // YAP: If old database is in load => shutdown & reconnect
        if (doCompact) {
            shutdown(true);
            mConn = DriverManager.getConnection(mUrl, mUser, mPassword);
        }
    ...
    public Connection getNewConnection() throws SQLException {
        ...
        for (int i = 0; i < 5; i++) {
            try {
                ...
                // YAP: Changed to log the message.
                LOGGER.warn("Recovering {} times.", i);
            }

            try {
                Thread.sleep(500);
                // YAP: Changed to catch the InterruptedException.
