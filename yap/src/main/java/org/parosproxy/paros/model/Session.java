...
// YAP: 2012/02/11 Re-ordered icons, added spider icon and notify via SiteMap
// YAP: 2012/02/18 Rationalised session handling
// YAP: 2012/04/23 Added @Override annotation to all appropriate methods and
// removed unnecessary casts.
// YAP: 2012/05/15 Changed the method parse() to get the session description.
// YAP: 2012/06/11 Changed the JavaDoc of the method isNewState().
// YAP: 2012/07/29 Issue 43: Added support for Scope
// YAP: 2012/08/01 Issue 332: added support for Modes
// YAP: 2012/08/07 Added method for getting all Nodes in Scope
// YAP: 2012/08/29 Issue 250 Support for authentication management
...
// YAP: 2012/12/14 Issue 438: Validate regexs as part of API enhancements
// YAP: 2013/04/16 Issue 638: Persist and snapshot sessions instead of saving them
// YAP: 2013/08/27 Issue 772: Restructuring of Saving/Loading Context Data
// YAP: 2013/09/26 Issue 747: Error opening session files on directories with special characters
...
// YAP: 2014/01/31 Issue 979: Sites and Alerts trees can get corrupted - load session on
// EventDispatchThread
// YAP: 2014-02-04 Added GlobalExcludeURL functionality:  Issue: TODO - insert bug/issue list here.
// YAP: 2014/03/23 Issue 997: Session.open complains about improper use of addPath
// YAP: 2014/03/23 Issue 999: History loaded in wrong order
// YAP: 2014/05/26 Added listeners for contexts changed events.
// YAP: 2014/06/10 Added helper method for removing data for context and type
// YAP: 2014/07/15 Issue 1265: Context import and export
// YAP: 2014/11/18 Issue 1408: Extend the structural parameter handling to forms param
// YAP: 2014/12/22 Issue 1476: Display contexts in the Sites tree
// YAP: 2015/01/30 Set default context name
...
// YAP: 2015/08/19 Change to use YapXmlConfiguration instead of extending FileXML
// YAP: 2015/08/19 Issue 1789: Forced Browse/AJAX Spider messages not restored to Sites tab
// YAP: 2015/10/21 Issue 1576: Support data driven content
// YAP: 2015/12/14 Issue 2119: Context's description not imported
// YAP: 2016/02/26 Issue 2273: Clear stats on session init
// YAP: 2016/05/02 Issue 2451: Only a single Data Driven Node can be saved in a context
// YAP: 2016/05/04 Changes to address issues related to ParameterParser
// YAP: 2016/05/10 Use empty String for (URL) parameters with no value
// YAP: 2016/05/24 Call Database.discardSession(long) in Session.discard()
// YAP: 2016/06/10 Do not clean up the database if the current session does not require it
// YAP: 2016/07/05 Issue 2218: Persisted Sessions don't save unconfigured Default Context
// YAP: 2016/08/25 Detach sites tree model when loading the session
// YAP: 2016/08/29 Issue 2736: Can't generate reports from saved Session data
// YAP: 2016/10/24 Delay addition of imported context until it's known that it has no errors
// YAP: 2016/10/26 Issue 1952: Do not allow Contexts with same name
// YAP: 2016/12/06 Remove contexts before refreshing the UI when discarding the contexts
// YAP: 2017/01/04 Remove dependency on ExtensionSpider
// YAP: 2017/01/26 Remove dependency on ExtensionActiveScan
// YAP: 2017/03/13 Remove global excluded URLs from Session's state.
// YAP: 2017/06/07 Allow to persist the session properties (e.g. name, description).
// YAP: 2017/09/03 Cope with Java 9 change to TreeNode.children().
// YAP: 2017/10/11 Make contextsChangedListeners static.
// YAP: 2018/01/25 Do not save session file if not file based.
// YAP: 2018/02/14 Remove unnecessary boxing / unboxing
// YAP: 2018/07/09 No longer need cast on SiteMap.getRoot
...
// YAP: 2020/07/31 Tidy up parameter methods
// YAP: 2020/08/17 Changed to use the VariantFactory
// YAP: 2020/09/04 Added getContextDataString and getContextDataInteger
// YAP: 2020/10/01 Remove use of org.jfree.util.Log use normal log4j infrastructure.
// YAP: 2020/10/14 Require just the name when importing context.
// YAP: 2020/11/02 Validate parameters in getLeafName(...)
// YAP: 2020/11/26 Use Log4j 2 classes for logging.
// YAP: 2022/02/09 Remove code no longer needed and deprecate a method.
// YAP: 2022/02/28 Remove code deprecated in 2.6.0
// YAP: 2022/08/23 Use SiteMap#createTree to create a new Sites Tree when loading a session.
// YAP: 2022/09/21 Use format specifiers instead of concatenation when logging.
// YAP: 2023/01/10 Tidy up logger.
// YAP: 2023/05/21 Allow context import functionality to accept an XML config as input (Issue 7421).
// YAP: 2023/06/02 Allow to set the global exclude URLs.
...
import org.yaproxy.yap.control.ExtensionFactory;
import org.yaproxy.yap.model.Context;
import org.yaproxy.yap.model.IllegalContextNameException;
import org.yaproxy.yap.model.NameValuePair;
import org.yaproxy.yap.model.ParameterParser;
import org.yaproxy.yap.model.SessionStructure;
import org.yaproxy.yap.model.StandardParameterParser;
import org.yaproxy.yap.model.StructuralNodeModifier;
import org.yaproxy.yap.model.Tech;
...
public class Session {
    ...
    private YapXmlConfiguration configuration;
    ...
    public Session(Model model) {
        configuration = new YapXmlConfiguration();
    ...
    protected void open(String fileName) throws DatabaseException, IOException, Exception {
        ...
        if (Database.DB_TYPE_HSQLDB.equals(model.getDb().getType())) {
            configuration = new YapXmlConfiguration(new File(fileName));
    ...
    public void exportContext(Context c, File file) throws ConfigurationException {
        YapXmlConfiguration config = new YapXmlConfiguration();
    ...
    public Context importContext(File file)
            throws ConfigurationException,
                    ...
                    SecurityException {
        return importContext(new YapXmlConfiguration(file));
    }
     ...
     * @since 2.13.0
     */
    public Context importContext(YapXmlConfiguration config)
            throws ConfigurationException,
                    ClassNotFoundException,
                    InstantiationException,
                    IllegalAccessException,
    ...
    // YAP: Added listeners for contexts changed events.
