 ...
 */
// YAP: 2011/09/19 Handle multipart node name
// YAP: 2011/12/04 Support deleting alerts
...
// YAP: 2012/03/11 Issue 280: Escape URLs in sites tree
// YAP: 2012/03/15 Changed the methods getQueryParamString and createReference to
//      use the class StringBuilder instead of StringBuffer
// YAP: 2012/07/03 Issue 320: AScan can miss subtrees if invoked via the API
// YAP: 2012/07/29 Issue 43: Added support for Scope
// YAP: 2012/08/29 Issue 250 Support for authentication management
// YAP: 2013/01/29 Handle structural nodes in findNode
...
// YAP: 2014/04/10 Initialise the root SiteNode with a reference to SiteMap
// YAP: 2014/04/10 Allow to delete history ID to SiteNode map entries
// YAP: 2014/06/16 Issue 1227: Active scanner sends GET requests with content in request body
...
// YAP: 2014/11/27 Issue 1416: Allow spider to be restricted by the number of children
// YAP: 2014/12/17 Issue 1174: Support a Site filter
// YAP: 2015/02/09 Issue 1525: Introduce a database interface layer to allow for alternative
// implementations
// YAP: 2015/04/02 Issue 1582: Low memory option
// YAP: 2015/08/19 Change to cope with deprecation of
// HttpMessage.getParamNameSet(HtmlParameter.Type, String)
// YAP: 2015/08/19 Issue 1784: NullPointerException when active scanning through the API with a
// target without scheme
// YAP: 2015/10/21 Issue 1576: Support data driven content
// YAP: 2015/11/05 Change findNode(..) methods to match top level nodes
// YAP: 2015/11/09 Fix NullPointerException when creating a HistoryReference with a request URI
// without path
// YAP: 2016/04/21 Issue 2342: Checks non-empty method for deletion of SiteNodes via API
// YAP: 2016/04/28 Issue 1171: Raise site and node add or remove events
// YAP: 2016/07/07 Do not add the message to past history if it already belongs to the node
// YAP: 2017/01/23: Issue 1800: Alpha sort the site tree
// YAP: 2017/06/29: Issue 3714: Added newOnly option to addPath
// YAP: 2017/07/09: Issue 3727: Sorting of SiteMap should not include HTTP method (verb) in the
// node's name
// YAP: 2017/11/22 Expose method to create temporary nodes (Issue 4065).
// YAP: 2017/12/26 Remove redundant request header null checks.
// YAP: 2018/02/07 Set the HistoryReference into the temp node before adding it to the tree (Issue
// 4356).
// YAP: 2018/02/14 Remove unnecessary boxing / unboxing
// YAP: 2018/07/09 Override getRoot method
// YAP: 2019/06/01 Normalise line endings.
// YAP: 2019/06/05 Normalise format/style.
// YAP: 2020/07/31 Tidy up parameter methods
// YAP: 2020/08/17 Changed to use getTreePath(msg) method
// YAP: 2020/11/02 Do not get leaf name if finding branch nodes.
// YAP: 2020/11/26 Use Log4j 2 classes for logging.
// YAP: 2021/05/14 Remove empty statement and add missing override annotation.
// YAP: 2022/02/08 Use isEmpty where applicable.
// YAP: 2022/07/27 Use hrefMap to return early from addPath and findAndAddChild. Remove getHostName
// and use SessionStructure#getHostName in its place.
// YAP: 2022/08/05 Address warns with Java 18 (Issue 7389).
// YAP: 2022/08/23 Make hrefMap an instance variable.
...
import org.yaproxy.yap.view.SiteTreeFilter;
...
public class SiteMap extends SortedTreeModel {
    ...
    private SiteNode findAndAddChild(
        ...
        // YAP: Cope with getSiteNode() returning null
    ...
    private HistoryReference createReference(
        ...
        for (int i = 1; i < path.length; i++) {
            // YAP Cope with error counts in the node names
        ...
        // YAP: Prevents a possible URIException, because the passed string is not escaped.
