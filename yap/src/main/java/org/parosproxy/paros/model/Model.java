...
// YAP: 2012/05/02 Added the method createSingleton and changed the method
// getSingleton to use it.
// YAP: 2012/06/11 Changed the method copySessionDb to call the method
// Database.close(boolean, boolean).
// YAP: 2012/08/08 Check if file exist.
...
// YAP: 2013/11/16 Issue 881: Fail immediately if yapdb.script file is not found
...
// YAP: 2016/02/10 Issue 1958: Allow to disable database (HSQLDB) log
// YAP: 2016/03/22 Allow to remove ContextDataFactory
...
// YAP: 2018/03/27 Validate that context and configurations for ContextDataFactory are not null.
// YAP: 2018/07/19 Fallback to bundled yapdb.script file.
// YAP: 2018/08/15 Deprecated addSessionListener
// YAP: 2019/06/01 Normalise line endings.
// YAP: 2019/06/05 Normalise format/style.
// YAP: 2020/09/15 Added the VariantFactory
// YAP: 2020/10/14 Allow to set a singleton Model for tests.
...
import org.yaproxy.yap.db.sql.DbSQL;
import org.yaproxy.yap.extension.ascan.VariantFactory;
...
public class Model {
    ...
    public static Model getSingleton() {
        if (model == null) {
            // YAP: Changed to use the method createSingleton().
            createSingleton();
        }
        return model;
    }
    ...
    public void createAndOpenUntitledDb() throws ClassNotFoundException, Exception {
        ...
        for (int i = 0; i < listFile.length; i++) {
            if (!listFile[i].delete()) {
                // YAP: Log failure to delete file
        ...
        FileCopier copier = new FileCopier();
        File fileIn = new File(Constant.getYapInstall(), DBNAME_TEMPLATE + ".data");
        if (fileIn.exists()) {
        ...
        fileIn = new File(Constant.getYapInstall(), DBNAME_TEMPLATE + ".properties");
        if (fileIn.exists()) {
        ...
        fileIn = new File(Constant.getYapInstall(), DBNAME_TEMPLATE + ".script");
        if (fileIn.exists()) {
            ...
            String fallbackResource = "/org/yaproxy/yap/resources/yapdb.script";
