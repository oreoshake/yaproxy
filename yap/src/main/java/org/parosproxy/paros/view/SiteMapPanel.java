...
// YAP: 2012/03/15 Changed to initiate the tree with a default model. Changed to
// clear the http panels when the root node is selected.
// YAP: 2012/04/23 Added @Override annotation to all appropriate methods.
// YAP: 2012/06/13 Added custom tree cell renderer to treeSite in getTreeSite().
// YAP: 2013/01/25 Added method for removing listener.
...
// YAP: 2014/03/23 Tidy up, removed the instance variable rootTreePath, no need to
// cache the path
// YAP: 2014/03/23 Issue 609: Provide a common interface to query the state and
...
// YAP: 2015/06/01 Issue 1653: Support context menu key for trees
// YAP: 2016/04/14 Use View to display the HTTP messages
// YAP: 2016/07/01 Issue 2642: Slow mouse wheel scrolling in site tree
// YAP: 2017/03/28 Issue 3253: Facilitate exporting URLs by context (add getSelectedContext)
// YAP: 2017/09/02 Use KeyEvent instead of Event (deprecated in Java 9).
// YAP: 2017/11/01 Delete context with keyboard shortcut.
// YAP: 2017/11/16 Hide filtered nodes in macOS L&F.
// YAP: 2017/11/29 Delete site nodes with keyboard shortcut.
// YAP: 2017/12/22 Select context on row click.
// YAP: 2018/03/26 Expand node of selected context.
// YAP: 2018/04/12 Keep panel of selected context selected.
...
// YAP: 2019/09/09 Issue 3491: Add support for selecting multiple contexts
// YAP: 2020/11/26 Use Log4j 2 classes for logging.
// YAP: 2022/07/04 Make delete more consistent and protective (Issue 7336).
...
import org.yaproxy.yap.view.ContextCreateDialog;
import org.yaproxy.yap.view.ContextGeneralPanel;
import org.yaproxy.yap.view.ContextsSitesPanel;
import org.yaproxy.yap.view.ContextsTreeCellRenderer;
import org.yaproxy.yap.view.DeleteContextAction;
import org.yaproxy.yap.view.LayoutHelper;
import org.yaproxy.yap.view.SiteMapListener;
import org.yaproxy.yap.view.SiteMapTreeCellRenderer;
import org.yaproxy.yap.view.SiteTreeFilter;
import org.yaproxy.yap.view.YapToggleButton;
import org.yaproxy.yap.view.messagecontainer.http.DefaultSelectableHistoryReferencesContainer;
import org.yaproxy.yap.view.messagecontainer.http.SelectableHistoryReferencesContainer;
...
public class SiteMapPanel extends AbstractPanel {
    ...
    private YapToggleButton scopeButton = null;
    ...
    private JToggleButton getScopeButton() {
        if (scopeButton == null) {
            scopeButton = new YapToggleButton();
            scopeButton.setIcon(
                    DisplayUtils.getScaledIcon(
                            new ImageIcon(
                    ...
                    new javax.swing.event.TreeSelectionListener() {
                        ...
                        public void valueChanged(javax.swing.event.TreeSelectionEvent e) {
                            ...
                            if (!node.isRoot()) {
                                ...
                                // YAP: Call SiteMapListenners
                                for (SiteMapListener listener : listeners) {
                                    listener.nodeSelected(node);
                                }
                            } else {
                                // YAP: clear the views when the root is selected
    ...
    // YAP: Added addSiteMapListenners
    ...
    protected class SitesCustomPopupMenu extends JPopupMenu {
        ...
        public void show(Component invoker, int x, int y) {
            // YAP: Select site list item on right click / menu key
