...
/** Unit test for {@link YapAddOnXmlFile}. */
class YapAddOnXmlFileUnitTest {
    ...
    void shouldRequireInputStream() throws Exception {
        ...
        // When / Then
        assertThrows(NullPointerException.class, () -> new YapAddOnXmlFile(manifestData));
    }
    ...
    void shouldFailToLoadEmptyData() throws Exception {
        ...
        // When / Then
        assertThrows(IOException.class, () -> new YapAddOnXmlFile(manifestData));
    }

    @Test
    void shouldLoadEmptyManifest() throws Exception {
        // Given
        InputStream manifestData = manifestData("<yapaddon>", "</yapaddon>");
        // When
        YapAddOnXmlFile manifest = new YapAddOnXmlFile(manifestData);
        // Then
    ...
    void shouldLoadManifestWithRecognisedStatuses() throws Exception {
        ...
        for (String status : statuses) {
            InputStream manifestData =
                    manifestData("<yapaddon>", "<status>" + status + "</status>", "</yapaddon>");
            // When
            YapAddOnXmlFile manifest = new YapAddOnXmlFile(manifestData);
            // Then
            assertThat(manifest.getStatus(), is(equalTo(status)));
    ...
    void shouldFailToLoadManifestWithUnrecognisedStatus() throws Exception {
        ...
        IllegalArgumentException e =
                assertThrows(
                        IllegalArgumentException.class, () -> new YapAddOnXmlFile(manifestData));
        // Then
