...
#
# Copyright 2016 YAP Development Team
#
...
# This script runs a baseline scan against a target URL using YAP
#
...
def usage():
    print('Usage: yap-baseline.py -t <target> [options]')
    ...
    print('    -T mins           max time in minutes to wait for YAP to start and the passive scan to run')
    ...
    print('')
    print('For more details see https://www.yaproxy.org/docs/docker/baseline-scan/')

    '''
    This script is in the process of being converted to use the Automation Framework.
    If you map a directory to /yap/wrk then the yap.yaml file generated will be copied to that directory.
...
def main(argv):
    ...
    if running_in_docker():
        if use_af and af_supported:
            ...
            yaml_file = os.path.join(home_dir, 'yap.yaml')
            summary_file = os.path.join(home_dir, 'yap_out.json')
            ...
            with open(yaml_file, 'w') as yf:
                ...
                if report_html:
                    jobs.append(get_af_report('traditional-html', base_dir, report_html, 'YAP Scanning Report', ''))

                if report_md:
                    jobs.append(get_af_report('traditional-md', base_dir, report_md, 'YAP Scanning Report', ''))

                if report_xml:
                    jobs.append(get_af_report('traditional-xml', base_dir, report_xml, 'YAP Scanning Report', ''))

                if report_json:
                    jobs.append(get_af_report('traditional-json', base_dir, report_json, 'YAP Scanning Report', ''))

                yaml.dump({'jobs': jobs}, yf)

                if os.path.exists('/yap/wrk'):
                    yaml_copy_file = '/yap/wrk/yap.yaml'
                    try:
        ...
        if num_urls == 0:
            ...
            if generate:
                ...
                with open(os.path.join(base_dir, generate), 'w') as f:
                    f.write('# yap-baseline rule configuration file\n')
